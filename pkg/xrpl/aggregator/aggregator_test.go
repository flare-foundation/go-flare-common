...
func TestAddSignaturesFull(t *testing.T) {
	...
	acc := Account{
		Address:    "rEuLyBCvcw4CFmzv8RepSiAoNgF8tTGJQC",
		SignerList: map[string]bool{"rsA2LpzuawewSBQXkiju3YQTMzW13pAAdW": true, "rUpy3eEg8rqjqfUoLeBnZkscbKbFsKXC3v": true},
		Quorum:     2,
		txs:        make(map[common.Hash]*transaction),
	}
	...
	require.Len(t, transaction.signers, 2, "signatures")
	require.Len(t, acc.txs, 1, "transactions")

	txBlob, err := acc.Finalize(transaction.id)
	require.NoError(t, err)
