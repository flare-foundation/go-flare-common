...
// Register is a paid mutator transaction binding the contract method 0xd2b4d762.
//
...
// MachineManagerChallengeInvalidatedIterator is returned from FilterChallengeInvalidated and is used to iterate over the raw logs and unpacked data for ChallengeInvalidated events raised by the MachineManager contract.
type MachineManagerChallengeInvalidatedIterator struct {
	Event *MachineManagerChallengeInvalidated // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *MachineManagerChallengeInvalidatedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(MachineManagerChallengeInvalidated)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *MachineManagerChallengeInvalidatedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *MachineManagerChallengeInvalidatedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// MachineManagerChallengeInvalidated represents a ChallengeInvalidated event raised by the MachineManager contract.
type MachineManagerChallengeInvalidated struct {
	TeeId common.Address
	Raw   types.Log // Blockchain specific contextual infos
}
...
func (_MachineManager *MachineManagerFilterer) FilterChallengeInvalidated(opts *bind.FilterOpts, teeId []common.Address) (*MachineManagerChallengeInvalidatedIterator, error) {
	...
	logs, sub, err := _MachineManager.contract.FilterLogs(opts, "ChallengeInvalidated", teeIdRule)
	if err != nil {
		return nil, err
	}
	return &MachineManagerChallengeInvalidatedIterator{contract: _MachineManager.contract, event: "ChallengeInvalidated", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event ChallengeInvalidated(address indexed teeId)
func (_MachineManager *MachineManagerFilterer) WatchChallengeInvalidated(opts *bind.WatchOpts, sink chan<- *MachineManagerChallengeInvalidated, teeId []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _MachineManager.contract.WatchLogs(opts, "ChallengeInvalidated", teeIdRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(MachineManagerChallengeInvalidated)
				if err := _MachineManager.contract.UnpackLog(event, "ChallengeInvalidated", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event ChallengeInvalidated(address indexed teeId)
func (_MachineManager *MachineManagerFilterer) ParseChallengeInvalidated(log types.Log) (*MachineManagerChallengeInvalidated, error) {
	event := new(MachineManagerChallengeInvalidated)
	if err := _MachineManager.contract.UnpackLog(event, "ChallengeInvalidated", log); err != nil {
		return nil, err
	}
...
// FilterTeeMachineRegistered is a free log retrieval operation binding the contract event 0xd0ebbbec57a0569c27525e2283a0f231dd1e3d2c0f8c7ce9e09ae897baa4a8de.
//
...
// WatchTeeMachineRegistered is a free log subscription operation binding the contract event 0xd0ebbbec57a0569c27525e2283a0f231dd1e3d2c0f8c7ce9e09ae897baa4a8de.
//
// Solidity: event TeeMachineRegistered(address indexed teeId, address indexed teeProxyId, address indexed owner, uint256 extensionId, string url, bytes32 codeHash, bytes32 platform, bytes32 governanceHash)
...
// ParseTeeMachineRegistered is a log parse operation binding the contract event 0xd0ebbbec57a0569c27525e2283a0f231dd1e3d2c0f8c7ce9e09ae897baa4a8de.
//
// Solidity: event TeeMachineRegistered(address indexed teeId, address indexed teeProxyId, address indexed owner, uint256 extensionId, string url, bytes32 codeHash, bytes32 platform, bytes32 governanceHash)
